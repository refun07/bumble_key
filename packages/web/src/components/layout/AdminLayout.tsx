            ...
            <div className="flex-1 lg:ml-72 flex flex-col min-h-screen transition-all duration-300">
                ...
                <header className={`backdrop-blur-md border-b h-20 flex items-center justify-between px-4 lg:px-10 sticky top-0 z-30 transition-colors duration-300 ${isDarkMode ? 'bg-[#0F0F0F]/80 border-zinc-800' : 'bg-white/80 border-gray-100'}`}>
                    <div className="flex items-center gap-2 lg:gap-8 flex-1">
                        ...
                        <h2 className={`text-lg lg:text-xl font-bold tracking-tight truncate ${isDarkMode ? 'text-white' : 'text-gray-900'} ${isSearching || searchQuery ? 'hidden sm:block' : 'block'}`}>
                            {navigation.find(n => location.pathname.startsWith(n.href))?.name || 'Admin'}
                        </h2>

                        {/* Search Bar - Responsive */}
                        <div className={`relative flex-1 max-w-md ${isSearching || searchQuery ? 'block' : 'hidden md:block'}`} ref={searchRef}>
                            <div className={`flex items-center rounded-2xl px-4 py-2 group transition-all ${isDarkMode ? 'bg-zinc-800 border border-zinc-700 focus-within:ring-2 focus-within:ring-bumble-yellow/20 focus-within:border-bumble-yellow' : 'bg-gray-50 border border-gray-100 focus-within:ring-2 focus-within:ring-bumble-yellow/20 focus-within:border-bumble-yellow'}`}>
                                        ...
                                        <div className="py-2 max-h-[60vh] overflow-y-auto">
                                            {searchResults.map((result, idx) => (
                                                <div
                                                    ...
                                                    className={`px-4 py-3 cursor-pointer flex items-center gap-3 transition-colors ${isDarkMode ? 'hover:bg-zinc-700' : 'hover:bg-gray-50'}`}
                                                >
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className={`text-sm font-bold truncate ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{result.title}</p>
                        ...
                        <button
                            onClick={() => {
                                if (searchQuery) {
                                    ...
                                    // Focus search input if we were to implement a separate mobile search state
                                    // For now, we just toggle the visibility via the searchQuery state logic
                                    setSearchQuery(' '); // Trigger search bar visibility
                                }
                            }}
                            className={`md:hidden p-2 rounded-xl transition-all ${isDarkMode ? 'text-zinc-400 hover:bg-zinc-800' : 'text-gray-400 hover:bg-gray-50'} ${searchQuery ? 'hidden' : 'block'}`}
                        >
                    ...
                    <div className="flex items-center gap-1 lg:gap-3 ml-4">
                        ...
                        <div className={`hidden sm:block h-8 w-px mx-1 lg:mx-2 ${isDarkMode ? 'bg-zinc-700' : 'bg-gray-100'}`}></div>

                        {/* User Menu Dropdown */}
                        <div className="relative" ref={userMenuRef}>
                            <button
                                onClick={() => setShowUserMenu(!showUserMenu)}
                                className={`flex items-center gap-2 lg:gap-3 pl-1 lg:pl-2 pr-1 lg:pr-3 py-1.5 rounded-2xl transition-all ${isDarkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-50'}`}
                            >
                                ...
                                <ChevronDownIcon className={`hidden sm:block h-4 w-4 transition-transform ${showUserMenu ? 'rotate-180' : ''} ${isDarkMode ? 'text-zinc-400' : 'text-gray-400'}`} />
                            </button>
                            ...
                            {showUserMenu && (
                                <div className={`absolute right-0 top-full mt-2 w-56 rounded-2xl shadow-xl border overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200 ${isDarkMode ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-gray-100'}`}>
                                    <div className="py-2">
                                        <div className="px-4 py-2 md:hidden border-b border-gray-50 dark:border-zinc-700 mb-1">
                                            <p className={`text-sm font-bold truncate ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{user?.name || 'Admin'}</p>
